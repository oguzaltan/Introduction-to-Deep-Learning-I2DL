...
class LSTM(nn.Module):
    def __init__(self, input_size=1, hidden_size=20):
        ...
        # cell gate
        self.W_g = nn.Linear(self.input_size, self.hidden_size, bias=True)
        self.U_g = nn.Linear(self.hidden_size, self.hidden_size, bias=True)
    ...
    def forward(self, x, h=None, c=None):
        ...
        seq_len, _, _ = x.size() # get the sequence length from input x
         
        # Iterate over all the time steps of input x
        for t in range(seq_len):
            
            # x has shape (seq_len, batch_size, input_size)
            # to get the data at the time step t (i.e. x_t) from the input x,
            # we take t-th data as the first parameter, so -> x[t, :, :]
            x_t = x[t, :, :] # shape -> (batch_size, input_size)
            
            # LSTM inputs and outputs 
            input_gate = self.sigmoid(self.W_i(x_t) + self.U_i(h))
            forget_gate = self.sigmoid(self.W_f(x_t) + self.U_f(h))
            output_gate = self.sigmoid(self.W_o(x_t) + self.U_o(h))
            cell_gate = self.tanh(self.W_g(x_t) + self.U_g(h))
          
            # Update the current hidden state and cell state
            # using hidden state and cell state from previous time step,
            # as well as gate outputs
            c = forget_gate * c + input_gate * cell_gate 
            h = output_gate * self.tanh(c)
          
            # Keep the hidden state history
            h_seq.append(h) # save current hidden state
            c_seq.append(c) # save current cell state
        
        # print(len(h_seq), h_seq)
        
        # Convert hidden state history and cell state history to tensors
        h_seq = torch.cat(h_seq, dim=0)
        c_seq = torch.cat(c_seq, dim=0)
        
        # print(h_seq.shape, h_seq)
...
class Embedding(nn.Module):
    def __init__(self, num_embeddings, embedding_dim, padding_idx):
        ...
        self.weight = nn.Parameter(torch.randn(size=(num_embeddings, embedding_dim)))
