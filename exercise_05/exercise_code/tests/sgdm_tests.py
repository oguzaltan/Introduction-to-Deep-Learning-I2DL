...
class SGDM_Weight_Test(UnitTest):
    ...
    def define_failure_message(self):
        return string_utils.failure_message(self.test_name, f"SGD Momentum Weight updates is incorrect. Expected: s < 1e-6,\
...
class SGDM_Velocity_Test(UnitTest):
    ...
    def define_failure_message(self):
        return string_utils.failure_message(self.test_name, f"SGD Momentum Velocity Values is incorrect. Expected: dw < 1e-6,\
            but evaluated: {self.error}")
...
class SGDMTestWrapper:
    ...
    def __call__(self, *args, **kwargs):
        test_results_to_score(self.sgdm_tests())
