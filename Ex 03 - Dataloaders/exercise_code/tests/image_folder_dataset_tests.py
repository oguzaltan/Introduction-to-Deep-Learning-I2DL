...
class GetItemTestType(UnitTest):
    ...
    def define_failure_message(self):
        return " ".join(f"{self.test_name} {self.failed_msg} {string_utils.ARROW} \
            Expected __getitem()__ to return type {self.type} but got {self.wrong_type}.".split())
...
class GetItemTestImageShape(UnitTest):
    ...
    def define_failure_message(self):
        return " ".join(f"{self.test_name} {self.failed_msg} {string_utils.ARROW} \
            Expected images to have shape {self.expected_shape} but got {str(self.dataset.images[0].shape)}.".split())
...
class GetItemTestTransformApplied(UnitTest):
    ...
    def define_failure_message(self):
        return " ".join(f"{self.test_name} {self.failed_msg} {string_utils.ARROW} \
            Expected images to be successfully transformed but the input image's first\
            entries are {str(self.dataset_entries)} and should be {str(self.loaded_entries)}.".split())
