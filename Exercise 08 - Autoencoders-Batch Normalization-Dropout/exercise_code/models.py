...
class Encoder(nn.Module):
    ...
    def __init__(self, hparams, input_size=28 * 28, latent_dim=20):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(self.input_size, self.latent_dim),
            nn.ReLU(),            
        )
...
class Autoencoder(nn.Module):
    ...
    def training_step(self, batch, loss_func):
        ...
        x = self.forward(batch) # forward
        
        loss = loss_func(x, batch) # calculate loss
...
class Classifier(nn.Module):
    ...
    def set_optimizer(self):
        ...
        # self.optimizer = torch.optim.Adam(self.model.parameters(), self.hparams["learning_rate"])
